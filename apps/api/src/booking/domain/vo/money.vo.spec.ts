...
/**
 * KEN'S EXECUTABLE SPEC for the Booking `Money` VO. Pure unit — ZERO mocks. RED
 * until you implement `money.vo.ts`. Amount is integer minor units (cents), ≥ 0.
 */
describe('Money (booking VO)', () => {
  ...
  it('rejects emtpy currency', () => {
    expect(() => Money.create(100, '')).toThrow();
  });

  it('reconstitutes a trusted stored amount', () => {
  ...
  it('compares by value (amount AND currency)', () => {
    expect(Money.create(100, 'USD').equals(Money.create(100, 'USD'))).toBe(
      true,
    );
    expect(Money.create(100, 'USD').equals(Money.create(200, 'USD'))).toBe(
      false,
    );
    expect(Money.create(100, 'USD').equals(Money.create(100, 'EUR'))).toBe(
      false,
    );
  });
});
