...
/**
 * BC-2 `DateRange` VO spec (scaffold-owned, GREEN now). Pins the half-open
 * `[checkIn, checkOut)` semantics the overbooking model depends on.
 */
describe('DateRange (inventory VO)', () => {
  describe('create', () => {
  ...
  describe('nights', () => {
    it('counts calendar nights between the bounds', () => {
      expect(DateRange.create(d('2026-07-01'), d('2026-07-04')).nights()).toBe(3);
      expect(DateRange.create(d('2026-07-01'), d('2026-07-02')).nights()).toBe(1);
    });
  });
